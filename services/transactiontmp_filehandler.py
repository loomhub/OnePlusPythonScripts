...
class transactiontmpFileHandler(myFileHandler):
...
#######################################################################################################################    
    def convert_df_to_list(self, df:pd.DataFrame,**kwargs) -> transactionsListDTO:
        ...
        df['period_status'] = self.period_status
...
# #################################################################################################################
    def convert_delDF_to_list(self, df:pd.DataFrame,**kwargs) -> transactionsListDTO:
        column_names = kwargs.get('column_names', None)
        rename_columns = kwargs.get('rename_columns', None) #rename columns for Chase downloads
